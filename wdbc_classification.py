...
def load_wdbc_data(filename):
    ...
    with open(filename) as f:
        for line in f.readlines():
            ...
            data_row = [float(i) for i in items[2:]]
            wdbc.data.append(data_row)         # TODO #1) Add 30 attributes (as floating-point numbers)
...
if __name__ == '__main__':
    ...
    # Train a model
    model = svm.SVC(kernel='rbf')                           # TODO #2) Find a better classifier (SVC accuracy: 0.902)
    ...
    conf_matx = metrics.confusion_matrix(wdbc.target, predict, labels=[0, 1])          
